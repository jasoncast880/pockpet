...
#	@rdagger-ili-series repo: refactored
  def load_sprite( self, path, aSize) :
    """ return a buffer containing the sprite pixel data
    ARGS:
    path(string): image file path
    aSize: (w, h)(int): width and height
    """
    buffer = bytearray()
    buf_size = 1
...
#	@rdagger-ili-series repo: refactored
  def draw_sprite( self, buf, aSize, aPos ) :
    #_setwindowloc( self, aPos0, aPos1 ) from top left to bottom right coords
    self._setwindowloc((aPos[0],aPos[1]), (aPos[0]+aSize[0],aPos[1]+aSize[1]))
    chunk_size = 3
    for i in range(0, len(buf), chunk_size):
        chunk = buf[i:i+chunk_size]
        self._pushcolor(TFTColor(chunk[2],chunk[1],chunk[0]))
...
#   @micropython.native
  ...
  def _pushcolor( self, aColor ) :
    if aColor == 0b0000000000011111:
      #print("f")
      self.colorData[0] = 0b00000000
      self.colorData[1] = 0b00000000
      self._writedata(self.colorData)
