...
Threaded version of bouncing boxes w frame buffer

Using simple flag for thread sync
...
class Box(object):
    ...
    def __init__(self, screen_width, screen_height, size, color):
        ...
        Args:
            screen_width(int): size of width 
            screen_height(int): size of height
            size(int): Sq. side length.
            display ({display-object}): display object
            color(int): RGB565 color val
        """
        ...
        #Generate non-zero random speeds bw -5.0 and 5.0
        seed(ticks_cpu())
        r = random() * 10.0
        self.x_speed = r - 5
        r = random() * 10.0
...
screen_width = #some num
screen_height = #240
screen_rotation = #some num
...
display = Display(spi, dc = Pin(15), cs = Pin(17), rst = Pin(14), width = screen_width, height = screen_height, rotation = screen_rotation)

print(spi)

#FrameBuffer needs 2 pytes per RGB565 pixel
buffer_width = #num
buffer_height = #num
...
def main_loop():
    ...
    try:
        boxes = [Box(buffer_width - 1, buffer_height - 1, randint(7,40),
                     color565(randint(30,256), randint(30,256),
                     randint(30,256))) for i in range(100)]
...
def render_thread(id):
    global fbuf, buffer, buffer_width, bufferheight, spi
    global buffer_lock
    ...
    while True:
        ...
        gc.collect() #run gc to prevent data corruption
